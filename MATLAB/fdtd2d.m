function [et,x,y] = fdtd2d(DATA,PAR)
...
%       nsps:   Number of source per side of the domain.
%       s_indx: Index of the source. The maximum allowed index is 4*nsps.
%               The sources distributed in a clockwise order, beginning
%               with the left side. The sources are equally spaced in each
%               side.
%
...
%   Institution of Engineering and Technology.
    ...
    % General parameters 
    number_of_time_steps            = PAR.nts;                              % Maximum number of time steps to run FDTD simulation.
    ...
    lsx                             = .1*lx;                                % Size of the source region between object and cpml in the x-direction.
    lsy                             = .1*ly;                                % Size of the source region between object and cpml in the x-direction.
    ...
    switch ceil(PAR.s_indx/PAR.nsps)
        case 1
            impressed_J(1).min_x = 0-lsx/2-dx;
            impressed_J(1).max_x = 0-lsx/2+dx;
            impressed_J(1).min_y = PAR.s_indx * ly / (PAR.nsps+1) - dy;
            impressed_J(1).max_y = PAR.s_indx * ly / (PAR.nsps+1) + dy;
        case 2
            impressed_J(1).min_x = (PAR.s_indx-PAR.nsps) * lx ...
                / (PAR.nsps+1) - dx;
            impressed_J(1).max_x = (PAR.s_indx-PAR.nsps) * lx ...
                / (PAR.nsps+1) + dx;
            impressed_J(1).min_y = ly+lsy/2-dy;
            impressed_J(1).max_y = ly+lsy/2+dy;
        case 3
            impressed_J(1).min_x = lx+lsx/2-dx;
            impressed_J(1).max_x = lx+lsx/2+dx;
            impressed_J(1).min_y = ly - (PAR.s_indx-2*PAR.nsps) * ly ...
                / (PAR.nsps+1) - dy;
            impressed_J(1).max_y = ly - (PAR.s_indx-2*PAR.nsps) * ly ...
                / (PAR.nsps+1) + dy;
        case 4
            impressed_J(1).min_x = lx-(PAR.s_indx-3*PAR.nsps) * lx ...
                / (PAR.nsps+1) - dx;
            impressed_J(1).max_x = lx-(PAR.s_indx-3*PAR.nsps) * lx ...
                / (PAR.nsps+1) + dx;
            impressed_J(1).min_y = 0-lsy/2-dy;
            impressed_J(1).max_y = 0-lsy/2+dy;
    end

    % Defining source parameters
    impressed_J(1).direction = 'zp';
    impressed_J(1).magnitude = 1e2;
    impressed_J(1).waveform_type = 'gaussian';
    impressed_J(1).waveform_index = 1;

    % Initializig domain sizes
    fdtd_domain.min_x = 0-lsx;
    fdtd_domain.max_x = lx+lsx;
    fdtd_domain.min_y = 0-lsy;
    fdtd_domain.max_y = ly+lsy;
    ...
    for time_step = 1:number_of_time_steps
        ...
        for f_indx = 1:length(PAR.f)
            et(:,:,f_indx) =  et(:,:,f_indx) + ...
                Ez*exp(-1j*2*pi*alpha(f_indx)*...
                (time_step-1)/number_of_time_steps);
        end
        
    end

    % Last step to perform Fourier transform
    et = 2*et/number_of_time_steps;
