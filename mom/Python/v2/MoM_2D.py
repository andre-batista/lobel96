...
Ei = E0*np.exp(-1j*kb*(x.reshape((-1,1))@np.cos(phi_i.reshape((1,-1))) 
                       + y.reshape((-1,1))@np.sin(phi_i.reshape((1,-1)))))
b = np.tile(Xr.reshape((-1,1)),(1,Ni))*Ei 
...
xg = np.tile(xs.reshape((-1,1)),(1,N*N))
yg = np.tile(ys.reshape((-1,1)),(1,N*N))

Rscat = np.sqrt((xg-np.tile(np.reshape(x,(N*N,1)).T,(Ns,1)))**2
                + (yg-np.tile(np.reshape(y,(N*N,1)).T,(Ns,1)))**2) # Ns x N*N
