...
# Compute Matrix-vector product by using two-dimensional FFT
    ...
    J = np.reshape(J,(N,N,Ni),order='F')
    Z = np.tile(Z[:,:,np.newaxis],(1,1,Ni))
    e = fft.ifft2(fft.fft2(Z,axes=(0,1))*fft.fft2(J,axes=(0,1),s=(2*N-1,2*N-1)),axes=(0,1))
    e = e[:N,:N,:]
    e = np.reshape(e,(N*N,Ni),order='F')
    e = np.reshape(J,(N*N,Ni),order='F') + np.tile(Xr.reshape((-1,1),order='F'),(1,Ni))*e
...
# *complex conjugate operator
    ...
    e = e[:N,:N,:]
    e = np.reshape(e,(N*N,Ni),order='F')
    e = np.reshape(J,(N*N,Ni),order='F') + np.conj(np.tile(Xr.reshape((-1,1),order='F'),(1,Ni)))*e
